...
PETRA.TransportPlan = function(gameState, HQ, units, startIndex, endIndex, endPos, ship)
{
...
PETRA.TransportPlan.prototype.assignShip = function(gameState)
{
	...
	let nearest;
	 this.HQ.navalManager.seaTransportShips[this.sea].forEach(ship => {
...
PETRA.TransportPlan.prototype.onBoarding = function(gameState)
{
	...
	if (this.needSplit)
	{
		 this.HQ.navalManager.splitTransport(gameState, this);
...
{
	if (! this.HQ.navalManager.landingZones[landIndex])
	{
		API3.warn(" >>> no landing zone for land " + landIndex);
		return destination;
	}
	else if (! this.HQ.navalManager.landingZones[landIndex][seaIndex])
	{
